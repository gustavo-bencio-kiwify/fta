...
async function interactive(app) {
        ...
        try {
            ...
            if (payload.type === "view_submission") {
                if (payload.view.callback_id === "create_task_modal") {
                    ...
                    const responsible = values.resp_block.responsible.selected_user;
                    // ✅ Prazo (string YYYY-MM-DD ou undefined)
                    const dueDate = values.due_block?.due_date?.selected_date;
                    // ✅ Urgência
                    const urgency = values.urgency_block.urgency.selected_option.value;
                    // ✅ CC (array de Slack IDs)
                    const carbonCopies = values.cc_block?.carbon_copies?.selected_users;
